...
float MainWindow::addOperation(float n1, float n2)
{
    ...
    //Inicializar guarda, sticky, round, operandos y complementado.

    unsigned int signoSuma, g = 0, r = 0, st = 0;
    bool interOp = false, cP = false;

    //Comprobar exponentes.
    ...
    //Calculamos exponente de la suma y de "d".

    int addExpo = expo1;
    unsigned int d = expo1 - expo2;

    //Comprobamos si los signos son diferentes.
    ...
    //Establecemos los bits de guarda, sticky y round.
    ...
    //Comprobamos de nuevo los signos.
    ...
    unsigned int C = acarreo(mantissa1, P, 0, 0);
    P += mantissa1;

    //Comprobamos el signo, la P y el acarreo.
    ...
    //Asimismo, comprobamos si el signo es el mismo y el acarreo es 1.
...
unsigned int MainWindow::acarreo(unsigned int mantissa1, unsigned int mantissa2, unsigned int position, unsigned int acarreoAc)
{
    ...
    if(acarreoAc != 0 && ((mantissa1 & bitPos.at(position)) != 0 || (mantissa2 & bitPos.at(position)) != 0)){
        return acarreo(mantissa1, mantissa2, position + 1, 1);
    }
...
void MainWindow::hexNumWrite(QLineEdit* objective, unsigned int signo, unsigned int expo, unsigned int mantissa)
{
    ...
    unsigned int chain = (expo << 23) + (mantissa) + (signo << 31);
    ...
    for(int i = 0; i < 8; i++)
    {
        ...
        if(hexBoolCheck(mod))
        {
            hexNum.push_front(hexCheck(mod));
        }else
        {
